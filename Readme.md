...
> ⚠️ The default config does leave a **server-side injection** attack vector open.
...
To close the server-side Datastar-attribute injection gap, we need to know that any given Datastar attribute was generated by _us_, not an attacker.
...
The `nonce` attribute gets special treatment from browsers: on pages with a header-delivered CSP, the value is moved out of the DOM once the element is inserted (readable only via the `el.nonce` IDL property), so CSS attribute-selector tricks can't exfiltrate it. If you'd rather use an ordinary attribute, set `Precompiler.NonceAttr` (e.g. to `"data-ds-nonce"`). The middleware tells the client which attribute to check, so no client-side configuration is needed. Do not mark nonce-stamped HTML fragments or pages as `public`/CDN-cacheable.

> ⚠️ The nonce does not guarantee that Datastar attribute values are safe. It only proves that the element was rendered through your trusted server-side template path. If a Datastar expression contains user-controlled data, that data still has to be escaped for the context where it is used. Be especially careful with user data inside JavaScript-like Datastar expressions such as `data-on:*`, `data-attr:*`, `data-show`, `data-text`, and action expressions. Prefer keeping user-controlled values outside Datastar expressions, for example in normal escaped text, inert `data-*` attributes, or server-generated JSON that is encoded for JavaScript.

Manually adding nonce attributes to your HTML is tedious. You'll probably want to find a way of setting up your templating system to automatically add the nonce attribute to any tag with other Datastar attributes.
...
**3.** Include a `Nonce` field in your template data and ensure that your templates add the required nonce attributes.
...
It might make sense to _selectively_ use nonces when rendering especially sensitive or risky pages. Nonce protection is opt-in at the middleware level. Just wrap `Middleware` with `NonceMiddleware` where you want it.
