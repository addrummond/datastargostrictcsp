...
<script
  type="module"
  src="https://cdn.jsdelivr.net/gh/addrummond/datastargostrictcsp@0.12.11/dist/datastargostrictcsp-client.lite.min.js"
></script>
```
...
Because `data-ds-nonce` is an ordinary attribute, browser parsing does not hide it the way browsers hide `<script nonce>` values. Treat nonce-bearing HTML as request-specific output: cache the signed precompile scripts if you like, but do not mark nonce-stamped HTML fragments or pages as `public`/CDN-cacheable unless your cache key is genuinely per request.

> ⚠️ `data-ds-nonce` does not guarantee that Datastar attribute values are safe. It only proves that the element was rendered through your trusted server-side template path. If a Datastar expression contains user-controlled data, that data still has to be escaped for the context where it is used. Be especially careful with user data inside JavaScript-like Datastar expressions such as `data-on:*`, `data-attr:*`, `data-show`, `data-text`, and action expressions. Prefer keeping user-controlled values outside Datastar expressions, for example in normal escaped text, inert `data-*` attributes, or server-generated JSON that is encoded for JavaScript.
