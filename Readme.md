...
![Datastar](https://img.shields.io/badge/Datastar-1.0.0-blueviolet?style=flat)
...
**2.** Load `dist/datastargostrictcsp-client.lite.js` via a  `<script type="module" src="...">` tag (without `defer` or `async`) and place it **before** the Datastar `<script type="module">` tag.
...
<img style="vertical-align:middle" alt="Gin" src="https://raw.githubusercontent.com/addrummond/datastargostrictcsp/main/middleware/gin/gin-icon.webp" width="24" alt="Gin">If you're using Gin, import the middleware/gin submodule and use dsgin.Adapt to convert any func(http.Handler) http.Handler middleware into a
`gin.HandlerFunc`:

```go
import dsgin "github.com/addrummond/datastargostrictcsp/middleware/gin"

router.Use(dsgin.Adapt(datastargostrictcsp.NonceMiddleware))
router.Use(dsgin.Adapt(pc.Middleware))
````
