...
func TestAddNonceToTemplate(t *testing.T) {
	const expr = "{{$.Nonce}}"

	tests := []struct {
		name     string
		input    string
		wantSub  string // must be present
		wantMiss string // must be absent
	}{
		{
			name:    "data-text element gets nonce",
			input:   `<div data-text="x"></div>`,
			wantSub: `data-ds-nonce="{{$.Nonce}}"`,
		},
		{
			name:    "data-on:click element gets nonce",
			input:   `<button data-on:click="doSomething()"></button>`,
			wantSub: `data-ds-nonce="{{$.Nonce}}"`,
		},
		{
			name:    "data-show element gets nonce",
			input:   `<div data-show="$visible"></div>`,
			wantSub: `data-ds-nonce="{{$.Nonce}}"`,
		},
		{
			name:     "plain div unchanged",
			input:    `<div class="foo"></div>`,
			wantMiss: `data-ds-nonce`,
		},
		{
			name:     "element already has data-ds-nonce: not overwritten",
			input:    `<div data-text="x" data-ds-nonce="existing"></div>`,
			wantSub:  `data-ds-nonce="existing"`,
			wantMiss: expr,
		},
		{
			name:     "data-ignore subtree not touched",
			input:    `<div data-ignore><span data-text="x"></span></div>`,
			wantMiss: `data-ds-nonce`,
		},
		{
			name:    "void element gets nonce",
			input:   `<input data-on:input="handle()">`,
			wantSub: `data-ds-nonce="{{$.Nonce}}"`,
		},
		{
			name:     "data-bind alone (dsAttrNone) does not get nonce",
			input:    `<input data-bind="mySignal">`,
			wantMiss: `data-ds-nonce`,
		},
		{
			name:    "original content is preserved",
			input:   `<div data-text="x">hello</div>`,
			wantSub: `>hello</div>`,
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := dscsp.AddNonceToTemplate(tc.input, expr)
			if tc.wantSub != "" && !strings.Contains(got, tc.wantSub) {
				t.Errorf("output %q\n  missing expected %q", got, tc.wantSub)
			}
			if tc.wantMiss != "" && strings.Contains(got, tc.wantMiss) {
				t.Errorf("output %q\n  should not contain %q", got, tc.wantMiss)
			}
		})
	}
}
